...
export default class GoogleProvider {
    ...
    buildAuthorizeUrl() {
        return `https://accounts.google.com/o/oauth2/v2/auth?scope=profile&include_granted_scopes=true&state=state_parameter_passthrough_value
            &redirect_uri=${this.redirectUrl}&response_type=token&client_id=${this.clientId}`
    }
    ...
    extractSession(redirectUrl) {
        ...
        console.log('extractSession', this.accessToken);
        return {
    ...
    getSignOutUrl(redirectUrl) {
        return `https://accounts.google.com/o/oauth2/revoke?token=${this.accessToken}`
    }
}